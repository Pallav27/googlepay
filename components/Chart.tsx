...
export default function TransactionChart({ transactions }: { transactions: Transaction[] }) {
  ...
  // Filter transactions for debits and credits
  const debitTransactions = transactions.filter((t) => t.debits > 0);
      ...
      <CardContent className="h-80">
        {showDebits ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={debitTransactions} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
          ...
          // Credits Chart
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={creditTransactions} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
              <XAxis dataKey="date" tick={{ fill: "#94a3b8" }} />
              <YAxis tick={{ fill: "#94a3b8" }} />
              <Tooltip
                contentStyle={{ backgroundColor: "#1e293b", color: "#f8fafc", borderRadius: "8px" }}
